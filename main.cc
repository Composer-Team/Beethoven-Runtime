  ...
  while (main_time < check_freq * 20) {
    ...
    if (((main_time >> 1) + 50) % check_freq == 0) {
      ...
      printf("\tCMDReady: %d\tRCU %d%d%d\tBusOccupied: %d\tCmdsInFlight: %d\tCProg %d",
             ongoing_cmd.ready_for_command, ongoing_rsp.state, ongoing_cmd.state, ongoing_update, bus_occupied,
             cmds_in_flight, ongoing_cmd.progress);
      fflush(stdout);
    }
    ...
    for (mem_interface<QData> &i: axi4_mems) {
      ...
      if (not i.b->send_ids.empty()) {
        ...
        *i.b->id = i.b->send_ids.front();
        printf("Setting b id to %d: %d\n", i.b->send_ids.front(), *i.b->id);
        if (*i.b->ready) {
          i.b->send_ids.pop();
        }
      } else {
